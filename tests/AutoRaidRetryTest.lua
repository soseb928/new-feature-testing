-- NexusPlay Auto Raid + Auto Retry TEST
    ...
    if RaidDataUpdated then
        table.insert(State.connections, RaidDataUpdated.OnClientEvent:Connect(function(data)
            ...
            if retryCount > 0 then
                State.retrying = true
                log("STATE | RetryingPlayers=" .. tostring(retryCount) .. " | RetryTime=" .. tostring(retryTime))
            end
...
local function start()
    ...
    task.spawn(function()
        ...
        while State.enabled do
            ...
            -- If retry did not transition into another raid, allow a fresh queue.
            if not State.inRaid and not State.retrying then
                invokeQueue("idle")
            end

            -- Once a retry transition has completed, the next RaidBossSpawned
            -- event sets inRaid=true again.
...
local function makeUI()
    ...
    title.Text = "Nexus Auto Raid TEST"
    title.TextSize = 18
    ...
    info.Text = "Megumi raid lifecycle test. Uses the game's existing queue/retry RemoteFunctions; no damage, teleport, or prompt manipulation."
    info.TextSize = 11
